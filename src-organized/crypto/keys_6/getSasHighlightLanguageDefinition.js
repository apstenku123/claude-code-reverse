/**
 * Returns the language definition object for SAS syntax highlighting.
 *
 * This function defines the keywords, built-ins, and syntax rules for SAS, intended for use with syntax highlighters such as highlight.js.
 *
 * @param {object} hljs - The highlight.js library object, providing string modes and comment modes.
 * @returns {object} The SAS language definition object for syntax highlighting.
 */
function getSasHighlightLanguageDefinition(hljs) {
  // List of SAS meta keywords (statements, options, etc.)
  ...
  return {
    ...
    keywords: {
      ...
      // SAS meta keywords (statements, options, etc.)
      meta: sasMetaKeywords
    },
    contains: [
      // Main SAS block keywords (proc, data, run, quit)
      {
        className: "keyword",
        begin: /^\s*(proc [\w\d_]+|data|run|quit)[\s;]/
      },
      // Macro variable references (e.g., &var, &var.)
      {
        className: "variable",
        begin: /&[a-zA-Z_&][a-zA-Z0-9_]*\.?/
      },
      // Emphasis for datalines/cards blocks
      {
        className: "emphasis",
        begin: /^\s*datalines|cards.*;/,
        end: /^\s*;\s*$/
      },
      // Built-in macro functions (e.g., %put, %let, etc.)
      {
        className: "built_in",
      ...
      // Built-in SAS functions (e.g., abs(), date(), etc.)
      {
        className: "meta",
        begin: new RegExp("[^%](" + sasBuiltInFunctions + ")[(]")
      },
...
module.exports = getSasHighlightLanguageDefinition;