/**
 * Renders and manages the UI for background bash shells, allowing users to view, select, and kill shells.
 * Handles shell selection, shell termination, and closing the shell manager UI.
 *
 * @param {Object} props - The component props.
 * @param {Function} props.onDone - Callback invoked when the shell manager should close (e.g., on escape key or cancel).
 * @returns {React.ReactElement|null} The rendered shell manager UI or null if a selected shell is not found.
 */
function BackgroundShellManager({ onDone }) {
  // Get theme styles for UI coloring
  const themeStyles = getThemeStylesheet(); // getThemeStylesheet

  // Get current shells and shell kill function
  const {
    shells,
    killShell
  } = useShellsState();
   ...
   * @param {string|number} shellId - The selected shell'createInteractionAccessor updateSnapshotAndNotify.
   */
  ...
  /**
   * Handles killing a shell by its updateSnapshotAndNotify.
   * @param {string|number} shellId - The shell'createInteractionAccessor updateSnapshotAndNotify to kill.
   */
  ...
  // Get pending exit key info (e.g., for double-press to exit)
  ...
  // Prepare shell options for selection list
    ...
    uG.default.createElement(
      ...
      exitKeyInfo.pending
        ? uG.default.createElement(
            _,
            { dimColor: true },
            "Press ", exitKeyInfo.keyName, " again to exit"
          )
        : uG.default.createElement(
            _,
            { dimColor: true },
...
module.exports = BackgroundShellManager;