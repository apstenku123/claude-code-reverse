/**
 * Renders and manages the UI flow for installing the Claude local package, including setup, aliasing, and uninstall steps.
 * Handles state transitions, side effects, and error reporting throughout the installation process.
 *
 * @returns {React.Element} The React element representing the current step of the local installer UI.
 */
function getLocalInstallerUI() {
  // State for current step, error message, and shell alias output
  ...
  // Handle user exit/cancel on unmount
  useCtrlKeyActionHandler(() => {
    pO("canceled", "user_exit");
    Q7(1);
  });

  // On mount, record the start of the process
  m2.useEffect(() => {
    pO("start");
  }, []);

  // Handle side effects for each step
  m2.useEffect(() => {
    /**
      ...
      try {
        ...
        switch (installResult) {
          ...
          case "install_failed":
            ...
            break;
          default:
            setErrorMessage("Unknown installation result");
            setCurrentStep("error");
...
module.exports = getLocalInstallerUI;