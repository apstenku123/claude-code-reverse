/**
 * Sets HEIF (High Efficiency Image File Format) encoding options on the current instance.
 * Validates and assigns provided options such as compression, quality, lossless, effort, chroma subsampling, and bit depth.
 ...
 * @param {number} [heifOptions.effort] - Encoding effort (integer between 0 and 9).
 ...
 * @throws {Error} If any option is invalid or not of the expected type/range.
 */
function setHeifOptions(heifOptions) {
  ...
  if (x1.string(heifOptions.compression)) {
    const validCompressions = ["av1", "hevc"];
    if (x1.inArray(heifOptions.compression, validCompressions)) {
  ...
  if (x1.defined(heifOptions.chromaSubsampling)) {
    const validChromaSubsampling = ["4:2:0", "4:4:4"];
    if (
      x1.string(heifOptions.chromaSubsampling) &&
      x1.inArray(heifOptions.chromaSubsampling, validChromaSubsampling)
    ) {
  ...
  if (x1.defined(heifOptions.bitdepth)) {
    const validBitdepths = [8, 10, 12];
    if (
      x1.integer(heifOptions.bitdepth) &&
      x1.inArray(heifOptions.bitdepth, validBitdepths)
    ) {
      // If bitdepth is not 8 and using prebuilt binaries, throw error
      if (
