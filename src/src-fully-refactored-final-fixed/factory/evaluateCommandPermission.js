/**
 * Evaluates whether a given command is allowed, denied, or requires user permission based on matching rules and read-only status.
 *
 * @param {Object} commandInput - The input object containing command details.
 * @param {Object} context - Additional context or configuration for rule evaluation.
 * @returns {Object} An object describing the permission decision, including behavior, messages, reasons, and suggestions.
 */
function evaluateCommandPermission(commandInput, context) {
  // Trim the command string for consistent rule matching
  const trimmedCommand = commandInput.command.trim();

  // Evaluate rules for the command: get matching deny and allow rules
  const {
    matchingDenyRules,
    matchingAllowRules
  ...
  // If the command is read-only, allow isBlobOrFileLikeObject by default
