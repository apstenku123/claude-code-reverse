/**
 * Attempts to establish a TCP connection to the specified host and port within a given timeout.
 * Resolves to true if the connection is successful, otherwise resolves to false.
 *
 * @async
 ...
 */
async function checkTcpPortConnectivity(host, port, timeout = 500) {
  try {
    return new Promise((resolve) => {
      // Create a TCP connection using the external BO6 function
      const socket = BO6({
        host: host,
        port: port,
        timeout: timeout
...
module.exports = checkTcpPortConnectivity;
