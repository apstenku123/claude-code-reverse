/**
 * Renders the API Key input panel, allowing the user to select between using an existing API key or entering a new one.
 * Handles keyboard navigation and input events for API key entry.
 *
 * @param {Object} props - The component props
 * @param {boolean} props.existingApiKey - Whether an existing API key is available
 * @param {boolean} props.useExistingKey - Whether the user has chosen to use the existing API key
 ...
 * @param {function} props.onSubmit - Callback for when the user submits the form
 * @param {function} props.onToggleUseExistingKey - Callback for toggling between using the existing key and entering a new one
 * @returns {React.ReactElement} The rendered API key input panel
 */
function ApiKeyInputPanel({
  existingApiKey,
  useExistingKey,
  apiKey,
  onApiKeyChange,
  ...
  // Keyboard event handler for up/down/enter
  D0((_, key) => {
    if (existingApiKey) {
      if (key.upArrow) {
  ...
  return iZ.default.createElement(
    ...
    iZ.default.createElement(
      ...
      // Option to use existing API key
      existingApiKey &&
        iZ.default.createElement(
          g,
          { marginBottom: 1 },
      ...
      // Input field for new API key (only if not using existing key or no existing key is present)
...
module.exports = ApiKeyInputPanel;
