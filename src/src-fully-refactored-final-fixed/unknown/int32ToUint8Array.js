/**
 * Converts a 32-bit unsigned integer into a 4-byte Uint8Array (big-endian order).
 *
 * @param {number} int32Value - The 32-bit unsigned integer to convert.
 * @returns {Uint8Array} a Uint8Array of length 4 representing the integer in big-endian order.
 */
function int32ToUint8Array(int32Value) {
  // Extract each byte from the 32-bit integer using bitwise operations
  // and arrange them in big-endian order (most significant byte first)
